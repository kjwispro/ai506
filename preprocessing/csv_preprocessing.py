...
DATA_DIR = '/home/ai506/dataset/'

def return_train_ui_matrix():
    user_itemset_data = np.loadtxt(DATA_DIR + 'user_itemset_training.csv', delimiter=',')
...
def return_valid_ui_data():
    query = np.loadtxt(DATA_DIR + 'user_itemset_valid_query.csv', delimiter=',')
    answer = np.loadtxt(DATA_DIR + 'user_itemset_valid_answer.csv', delimiter=',')
    data = np.concatenate([query, np.expand_dims(answer, axis=1)], axis=1)
    return data


def return_train_ii_matrix():
    itemset_item_data = np.loadtxt(DATA_DIR + 'itemset_item_training.csv', delimiter=',')
...
def return_valid_ii_data():
    query = np.loadtxt(DATA_DIR + 'itemset_item_valid_query.csv', delimiter=',')
    answer = np.loadtxt(DATA_DIR + 'itemset_item_valid_answer.csv', delimiter=',')
    return query, answer

def return_ui_matrix():
    user_item_data = np.loadtxt(DATA_DIR + 'user_item.csv', delimiter=',')
